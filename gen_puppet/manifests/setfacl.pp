...
define setfacl ($dir=$name, $acl, $make_default=false, $recurse=true, $mask='rwx') {
  ...
  if $make_default {
    ...
    exec {
      "Set default acls '${acl}' on ${dir}":
        ...
        unless      => "/usr/bin/test $(/usr/bin/getfacl --absolute-names ${dir} | /bin/grep '^default:${acl}')",
        timeout     => 0,
        require     => Package["acl"];
      "Set default acls '${acl}' on ${dir} (mask)":
        command     => "/usr/bin/setfacl ${real_r} -n -m default:mask:${mask} ${dir}",
        unless      => "/usr/bin/test $(/usr/bin/getfacl --absolute-names ${dir} | /bin/grep '^default:mask::${mask}')",
        timeout     => 0,
        require     => Package["acl"];
    }
  }
  ...
  exec {
    "Set acls '${acl}' on ${dir}":
      ...
      unless      => "/usr/bin/test $(/usr/bin/getfacl --absolute-names ${dir} | /bin/grep '^${acl}')",
      timeout     => 0,
      require     => Package["acl"];
    "Set acls '${acl}' on ${dir} (mask)":
      command     => "/usr/bin/setfacl ${real_r} -n -m mask:${mask} ${dir}",
      unless      => "/usr/bin/test $(/usr/bin/getfacl --absolute-names ${dir} | /bin/grep '^mask::${mask}')",
      timeout     => 0,
      require     => Package["acl"];
  }
}
